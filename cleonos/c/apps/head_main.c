    ...
    *out_input_len = 0ULL;
    ...
    if (file_arg != (const char *)0 && file_arg[0] != '\0') {
        if (ush_resolve_path(sh, file_arg, path, (u64)sizeof(path)) == 0) {
            ush_writeln_i18n("head: invalid path", "head: 无效路径");
            return 0;
        }

        if (cleonos_sys_fs_stat_type(path) != 1ULL) {
            ush_writeln_i18n("head: file not found", "head: 文件不存在");
            return 0;
        }
        ...
        if (size == (u64)-1) {
            ush_writeln_i18n("head: failed to stat file", "head: 获取文件信息失败");
            return 0;
        }

        if (size > (u64)USH_COPY_MAX) {
            ush_writeln_i18n("head: file too large for user buffer", "head: 文件过大，无法放入用户缓冲区");
            return 0;
        }
        ...
        if (got == 0ULL || got != size) {
            ush_writeln_i18n("head: read failed", "head: 读取失败");
            return 0;
        }
    ...
    if (ush_pipeline_stdin_text == (const char *)0) {
        ush_writeln_i18n("head: file path required (or pipeline input)", "head: 需要文件路径或管道输入");
        return 0;
    }
...
static int ush_cmd_head(const ush_state *sh, const char *arg) {
    ...
    if (ush_head_parse_args(arg, &line_count, file_arg, (u64)sizeof(file_arg)) == 0) {
        ush_writeln_i18n("head: usage head [-n N] [file]", "head: 用法 head [-n N] [file]");
        return 0;
    }
