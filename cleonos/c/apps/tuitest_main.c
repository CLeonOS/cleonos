...
static int tuitest_load_symbol(void *handle, const char *name, void **out) {
    *out = dlsym(handle, name);
    if (*out == (void *)0) {
        (void)printf((ush_locale_is_zh() != 0) ? "[tuitest] 缺少符号: %s\n"
...
static void tuitest_draw(const tuitest_api *tui, int focused_button, const char *last_key) {
    ...
    (void)puts((ush_locale_is_zh() != 0)
                   ? "| 已通过 dlopen/dlsym 加载 libtui 风格 API                                     |"
    ...
    (void)puts("|                                                                              |");
    (void)printf((ush_locale_is_zh() != 0) ? "| 最近按键: %-61s |\n" : "| Last key: %-66s |\n",
                 last_key);
    (void)puts("|                                                                              |");
    tuitest_print_line(tui, border, "+------------------------------------------------------------------------------+");
    tui->set_style(bar);
    (void)puts((ush_locale_is_zh() != 0)
                   ? " tuitest | 动态 TUI 库演示                          TAB 切换焦点 | q 退出 "
...
int cleonos_app_main(int argc, char **argv, char **envp) {
    ...
    if (handle == (void *)0) {
        (void)printf((ush_locale_is_zh() != 0) ? "[tuitest] dlopen 失败: %s\n"
    ...
    tui.shutdown();
    (void)dlclose(handle);
    ush_writeln_i18n("[tuitest] PASS", "[tuitest] 通过");
    return 0;
}
