...
int hbos_tek_getsize(unsigned char *p)
{
...
int hbos_tek_decomp(unsigned char *p, char *q, int size)
{
	int err = -1;
	if (*p == 0x83) {
...
static void tek_setbm5(struct tek_STR_BITMODEL *bm, int t, int m)
{
	bm->t = t;
	bm->m = m;
	bm->prb1 = ((UINT32)~0U) << (m + t);
	bm->prb0 = ~bm->prb1;
	bm->prb1 |= 1 << t;
	bm->tmsk = (((UINT32)~0U) << t) & 0xffffU;
	bm->prb0 &= bm->tmsk;
	bm->prb1 &= bm->tmsk;
	bm->ntm = ~bm->tmsk;
	return;
