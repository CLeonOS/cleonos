...
static void emoji_usage(void) {
    ...
    puts("examples: smile -> 😀, heart -> ❤️");
}

static void emoji_list(void) {
...
static int emoji_test_key(const char *key) {
    u64 size = cleonos_sys_fs_stat_size(EMOJI_DICT_PATH);
    char *data;
    u64 got;
    u64 pos = 0ULL;

    if (key == 0 || key[0] == '\0' || size == (u64)-1 || size == 0ULL) {
        puts("emoji: dictionary not available");
        return 1;
    }

    data = (char *)malloc((size_t)size + 1U);
    if (data == 0) {
        puts("emoji: out of memory");
        return 1;
    }

    got = cleonos_sys_fs_read(EMOJI_DICT_PATH, data, size);
    data[got] = '\0';
...
int cleonos_app_main(void) {
    ...
    if (strcmp(first, "use") == 0) {
        if (ush_parse_u64_dec(rest, &idx) == 0) {
            puts("emoji: invalid index");
            return 1;
        }
        if (cleonos_sys_inputm_select(idx) == 0ULL) {
            puts("emoji: select failed");
            return 1;
        }
        emoji_list();
        return 0;
